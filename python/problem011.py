table = [[0 for x in range(20)] for x in range(20)]


def insertInTable(line, rownum):
    "first Argument String, second number of the row"
    split = line.split()
    for x in range(20):
        table[x][rownum] = int(split[x])
...
print(str(len(lines)) + ' lines were read correctly')

# Creating Table - Array
for i in range(20):
    insertInTable(lines[i], i)
...
def checkUpDown():
    ...
    for x in range(20):
        for y in range(17):
            adder = table[x][y] * table[x][y+1] * table[x][y+2] * table[x][y+3]
            if adder > biggest:
                biggest = adder
    return(biggest)
...
def checkLeftRight():
    ...
    for y in range(20):
        for x in range(17):
            adder = table[x][y] * table[x+1][y] * table[x+2][y] * table[x+3][y]
            if adder > biggest:
                biggest = adder
    return(biggest)
...
def checkDiag():
    ...
    for y in range(4, 20, 1):
        for x in range(4, 20, 1):
            adder = table[x][y] * table[x-1][y-1] * table[x-2][y-2] * table[x-3][y-3]
            if adder > biggest:
                biggest = adder
    for y in range(4, 20, 1):
        for x in range(17):
...
biggestUpDown = checkUpDown()
biggestLeftRight = checkLeftRight()
biggestDiag = checkDiag()

if (biggestUpDown >= biggestLeftRight and biggestUpDown >= biggestDiag):
    print(biggestUpDown)
else:
    if (biggestLeftRight >= biggestDiag):
        print(biggestLeftRight)
    else:
        print(biggestDiag)
